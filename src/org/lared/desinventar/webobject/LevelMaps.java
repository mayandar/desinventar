...
public class LevelMaps extends webObject
{
	...
	public void updateHashTable() {
		// FIELD NAMES VECTOR
		asFieldNames.put("map_level", String.valueOf(map_level));
		asFieldNames.put("filename", filename);
		asFieldNames.put("lev_code", lev_code);
		asFieldNames.put("lev_name", lev_name);
	...
	public void updateMembersFromHashTable() {
		...
		setMap_level((String)asFieldNames.get("map_level"));
		setFilename((String)asFieldNames.get("filename"));
		setLev_code((String)asFieldNames.get("lev_code"));
		setLev_name((String)asFieldNames.get("lev_name"));
	...
	public void init() {
		...
		fieldNullState.put("map_level", new Boolean(true));
		map_level = 0;
		filename = "";
		lev_code = "";
		lev_name = "";
...
//--------------------------------------------------------------------------------
// Access methods for database field 'map_level'
	public String getMap_level() {
		return Integer.toString(map_level);
	}

	public void setMap_level(String sParameter) {
		map_level = extendedParseInt(sParameter);
		if (not_null(sParameter).length()>0) {
			fieldNullState.put("map_level", false);
		}
	}

	public void setMap_level(int sParameter) {
		map_level = sParameter;
		fieldNullState.put("map_level", false);
	}
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Access methods for database field 'lev_code'
	public String getLev_code() {
		return lev_code;
	}

	public void setLev_code(String sParameter) {
		lev_code = sParameter;
	}
//--------------------------------------------------------------------------------

//--------------------------------------------------------------------------------
// Access methods for database field 'lev_name'
	public String getLev_name() {
		return lev_name;
	}

	public void setLev_name(String sParameter) {
		lev_name = sParameter;
	}
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
	...
	public void loadWebObject(ResultSet rset) {
		try {
			...
			try {
				map_level = rset.getInt("map_level");

				fieldNullState.put("map_level", new Boolean(rset.wasNull()));
			} catch (Exception ex) {
				lastError = "<-- error attempting to access field map_level -->";
			...
			try {
				lev_code = not_null(rset.getString("lev_code"));

			} catch (Exception ex) {
				lastError = "<-- error attempting to access field lev_code -->";
				System.out.println(ex.toString());
			}

			try {
				lev_name = not_null(rset.getString("lev_name"));

			} catch (Exception ex) {
				lastError = "<-- error attempting to access field lev_name -->";
				System.out.println(ex.toString());
			}
	...
	public int getWebObject(Connection con) {
		try {		
			// SQL_GET
			int f=1;
			sSql = "SELECT * FROM level_maps";
			sSql += " WHERE (map_level = ?)";
			pstmt = con.prepareStatement(sSql);

			pstmt.setInt(f++, map_level);
  ...
  public int addWebObject(Connection con)
    {
		try {
			// SQL_INSERT
			int f=1;
			sSql = "insert into level_maps (";
			sSql += "map_level, filename, lev_code, lev_name, filetype, color_red";
			sSql += ", color_green, color_blue, line_thickness, line_type, projection_system";
			...
			if (lev_code==null || lev_code.length()==0)
				pstmt.setNull(f++, java.sql.Types.VARCHAR);
			else
				pstmt.setString(f++, lev_code);
			if (lev_name==null || lev_name.length()==0)
				pstmt.setNull(f++, java.sql.Types.VARCHAR);
			else
				pstmt.setString(f++, lev_name);
  ...
  public int updateWebObject(Connection con)
    {
		try {
			...
			if (map_level==0 && (Boolean)fieldNullState.get("map_level"))
				pstmt.setNull(f++, java.sql.Types.INTEGER);
			else
				pstmt.setInt(f++, map_level);
...
<tr><td>map_level:</td><td>  <INPUT type='TEXT' size='5' maxlength='22' name='map_level' VALUE="<%=beanName.map_level%>"></td></tr>
<tr><td>filename:</td><td>  <INPUT type='TEXT' size='50' maxlength='255' name='filename' VALUE="<%=beanName.filename%>"></td></tr>
<tr><td>lev_code:</td><td>  <INPUT type='TEXT' size='50' maxlength='50' name='lev_code' VALUE="<%=beanName.lev_code%>"></td></tr>
<tr><td>lev_name:</td><td>  <INPUT type='TEXT' size='50' maxlength='50' name='lev_name' VALUE="<%=beanName.lev_name%>"></td></tr>
