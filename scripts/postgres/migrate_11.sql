...
alter TABLE fichas alter column fuentes type varchar (255);

alter TABLE fichas add di_comments text;
update fichas set di_comments=coalesce(observa,'') || coalesce(observa2,'') || coalesce(observa3,'');

alter table fichas add uu_id varchar(37);
update fichas set uu_id=uuid_generate_v1();
