...
alter TABLE fichas alter column fuentes text (255)   NULL;

alter TABLE fichas add di_comments memo;
update fichas set di_comments=iif(isnull(observa),'',observa) & iif(isnull(observa2),'',observa2) & iif(isnull(observa3),'',observa3);


alter table fichas add uu_id text(37);
update fichas set uu_id=left(clave & int(rnd(clave)*9999999999)& (CDbl(Now())*9999999999+clave),26);
