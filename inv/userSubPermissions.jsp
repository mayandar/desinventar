...
else if (request.getParameter("addSubSubRegion")!=null)
   {
	%><jsp:forward page="userSubSubPermissions.jsp"/><%
   }
%>  
...
if (request.getParameter("removeRegion")!=null)
 if (request.getParameter("authorized")!=null)
   ...
   for (int j=0; j<asAuth.length; j++)
	...
	try{
		pstmt=con.prepareStatement("delete from user_subpermissions where userid=? and country=? and region_code like ? and region_level=?");
		pstmt.setString(1,permission.userid);
		pstmt.setString(2,permission.country);
		pstmt.setString(3,permission.region_code+"%");
		pstmt.setInt(4,1);
		pstmt.executeUpdate();		
	}
	catch (Exception e)
	{
	System.out.println("[DI9] Exception removing subpermision: "+e.toString());
	}
...
<form method="post" action="userSubPermissions.jsp" name="desinventar">
<input type="hidden" name="usrtkn" id="usrtkn" value="<%=countrybean.userHash%>"> 
	  ...
	  <tr>
	     <td align="center">
		 <strong><%=countrybean.getTranslation("Available Level 0")%>:</strong><br>
		 <SELECT name="available" style="WIDTH: 200px; HEIGHT: 166px; margin-left:25x" size=10 multiple>
	     <inv:selectLevelX connection="<%= con %>" user="<%=User.suserid%>" level="0" only_selected="N"/>
		 </SELECT>
		 </td>
		 <td align="center">
		 ...
		 <SELECT name="authorized_l" style="WIDTH: 200px; HEIGHT: 166px;  margin-left:25px" size=10 disabled>
      ...
      <td align="center" colspan='2'><INPUT type="submit" name="addSubSubRegion" value="<%=countrybean.getTranslation("Add or Remove Level 1")%>" onClick="return  bHasLevel0()"></td></tr>
