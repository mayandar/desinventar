...
String sDestination="UsersUpdate.jsp";
if (user.iusertype>=40) 
	sDestination="userManager.jsp";
...
if (request.getParameter("saveUser")!=null)
 if (sUserid.equals(User.suserid))
  { 
	User.getForm(request,response,con);
    User.updateWebObject(con);
	if (user.iusertype>=40) 
		{
		dbCon.close();
...
function giveFocus() 
{
  document.desinventar.sfirstname.focus()
}
...
{
var haveAlphaLow=false;
var haveAlphaCap=false;
var haveNum=false;
for (j=0; j<entered.value.length; j++)
	{
    toCheck=entered.value.charAt(j);
...
var validateFlag=haveNum && haveAlphaCap && haveAlphaLow;
if (!validateFlag)
	alert(alertbox);
return validateFlag;
}
...
<form name="desinventar" action="UsersUpdate.jsp" method="post" onSubmit="return validateFields()">
...
<tr><td width=180 class=bgLight align='right'><%=countrybean.getTranslation("Organization")%>:</td><td>  <INPUT type='TEXT' size='50' maxlength='50' name='sorganization' VALUE="<%=htmlServer.htmlEncode(User.sorganization)%>"><span class="warning">*</span></td></tr>
	...
	<TD valign="bottom">
	<input name="saveUser" type=submit value='<%=countrybean.getTranslation("Save")%>'> &nbsp; &nbsp; &nbsp; &nbsp;
<%if (user.iusertype>=40)  // only owner/superuser can return to user manager!!!
 {%>
	<input name="cancelUser" type=button value='<%=countrybean.getTranslation("Done")%>' onclick='postTo("<%=sDestination%>")'>
<%}%>
	</TD>
	</Tr>
